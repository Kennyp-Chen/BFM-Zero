...
class BaseTask(gym.Env):
    def __init__(self, config, device):
        ...
        if self.config.robot.control.get("normalize_action", False):
            self.single_action_space = gym.spaces.Box(low=-self.config.robot.control["normalize_action_from"], high=self.config.robot.control["normalize_action_from"], shape=(self.dim_actions,))
        else:
        ...
        self.single_critic_observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.dim_critic_obs,))
        self.critic_observation_space = gym.vector.utils.batch_space(self.single_critic_observation_space, self.num_envs)
    ...
    def reset_all(self, target_states=None):
        """ Reset all robots"""
        self.reset_envs_idx(torch.arange(self.num_envs, device=self.device), target_states=target_states)
        self.simulator.set_actor_root_state_tensor(torch.arange(self.num_envs, device=self.device), self.target_robot_root_states)
        self.simulator.set_dof_state_tensor(torch.arange(self.num_envs, device=self.device), self.target_robot_dof_state)
